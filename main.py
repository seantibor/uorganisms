...
class Organism(object):
    ...
    def __init__(
        ...
        if parent1 is None or parent2 is None:
            ...
            self.color = combine_traits(parent1.color, parent2.color, sort=False)
    ...
    def __hash__(self):
        if self.parent1 is None or self.parent2 is None:
            return hash(
                "{}{}{}".format(str(self.creation_time), self.gender, self.color)
            )
        else:
            return hash(
                "{}{}{}{}{}".format(
    ...
    def org_from_repr(cls, repr_string):
        ...
        return Organism(
            gender=repr_list[1],
            color=repr_list[2],
            parent1_hash=None if repr_list[3] == "None" else int(repr_list[3]),
            parent2_hash=None if repr_list[4] == "None" else int(repr_list[4]),
...
def load_organism(filename):
    ...
    with open(filename, "rt") as f:
        return Organism.org_from_repr(f.read())
...
try:
    ...
    print(
        "Organism {} loaded with {} gender and {} color.".format(
            hash(org), org.gender, org.color
        )
    )
    ...
    print(
        "Organism {} written with {} gender and {} color.".format(
            hash(org), org.gender, org.color
        )
    )
    print(repr(org))
